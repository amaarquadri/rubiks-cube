...
class ForwardStaticVector {
 ...
 private:
  explicit constexpr ForwardStaticVector(ConstexprSharedPtr<DataStore> data)
      : data(std::move(data)) {}

  ConstexprSharedPtr<DataStore> data;
...
class BackwardStaticVector {
 ...
 private:
  explicit constexpr BackwardStaticVector(ConstexprSharedPtr<DataStore> data)
      : data(std::move(data)) {}
 ...
 public:
  ...
  constexpr iterator end() {
    return data->values.rbegin() + data->backward_size;
  }
...
class BidirectionalStaticVector {
 ...
 public:
  static constexpr std::pair<ForwardVector, BackwardVector> make() {
    const auto data_store = ConstexprSharedPtr<DataStore>{new DataStore()};
    ForwardVector forward{data_store};
    BackwardVector backward{data_store};
